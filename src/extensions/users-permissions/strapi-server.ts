...
module.exports = (plugin) => {
  ...
  plugin.controllers.auth['register'] = async (ctx) => {
    ...
    try {
      ...
      var data = await axios.request(options1)
    ...
    try {
      const data1 = await axios.request(options)
      console.log(data1);
      return data1
  ...
  plugin.controllers.auth['login'] = async (ctx) => {
    ...
    try {
      const loggedIn = await axios.post(`${process.env.AUTH0_URL}/oauth/ro`, loginObject);
